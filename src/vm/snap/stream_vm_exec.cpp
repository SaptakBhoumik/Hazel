...
static std::uint64_t __attribute__((noinline, cold)) execute_cold_inst(std::uint64_t pc, StreamValue* code, StreamValue* frame_buffer) noexcept{
    ...
    _L_OP_PTR_ALLOC_I64_I64_II64:{
        ...
        const auto element_size = code[++pc];
    ...
    _L_OP_PUSH_PTR_PTR_I64_II64:{
        auto dest_ptr = frame_buffer + code[++pc].value;
        const auto src_ptr = frame_buffer[code[++pc].value];
        const auto num_elements = frame_buffer[code[++pc].value];
        const auto capacity_increase = code[++pc].value;
    ...
    _L_OP_MEMCPY_PTR_PTR_I64:{
        ...
        const auto no_of_item = frame_buffer[code[++pc].value];
...
std::int64_t execute_stream_vm(std::uint64_t frame_size, StreamValue* code, StreamValue* frame_buffer, std::int64_t* traceback_func_loc_idx,
    ...
    /* ---- Calls --------------------------------------- */
    _L_OP_CALL:{
        const auto func_loc = code[++pc].value;
        const auto stack_off = code[++pc].value;
        ...
        for(std::int64_t i = 0; i < num_args; i++){
            frame_buffer[stack_off + 1 + num_ret + i] = frame_buffer[code[++pc].value];
        }
    ...
    _L_OP_RET:{
        const auto num_ret = code[++pc].value;
        for(std::int64_t i = 0; i < num_ret; i++){
            frame_buffer[i] = frame_buffer[code[++pc].value];
        }
        traceback_func_loc_idx--;
        func_stack_off--;
        ret_loc_idx--;
